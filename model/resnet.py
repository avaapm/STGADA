...
class ResNet50Fc(nn.Module):
    ...
    def __init__(self, bottleneck_dim=256, class_num=100):
        ...
        self.lpf_weights = torch.Tensor(
            numpy.tile([[21., 31., 21.], [31., 48., 31.], [21., 31., 21.]], (2048, 1, 1, 1)) / 256)
    ...
    def forward(self, x, reverse=False, eta=0.1):
        ...
        x = self.layer3(x)
        x = self.layer4(x)
        # x.data = nn.functional.conv2d(x.data, self.lpf_weights.to(self.dummy.device), padding=1, groups=2048)
        x = self.avgpool(x)
        x = torch.flatten(x, 1)
        x = self.bottleneck(x)
        x = self.bn2(x)
